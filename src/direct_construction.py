...
class DirectAFDConstructor:
    ...
    def build_syntax_tree(self):
        ...
        if len(stack) != 1:
            raise ValueError(f"Error: Expresión postfix mal formada '{self.regex_postfix}', pila final: {len(stack)} elementos")
                ...
                # 📌 Mostrar followpos generado para depuración
        print("\n📌 Followpos generado:")
        for pos, follow in self.followpos.items():
            print(f"  Pos {pos}: {sorted(follow)}")
    ...
    def construct_afd(self):
        ...
        print("\n📌 Símbolos por posición:")
        for pos, sym in self.symbol_positions.items():
            print(f"  Posición {pos}: '{sym}'")
        ...
        while estados_pendientes:
            ...
            for pos in current_set:
                ...
                # Agrega esto antes del bucle for symbol, new_set...
            print(f"\n🔍 Analizando símbolo -> posiciones desde estado {current_state.id}:")
            for sym, pos_set in symbol_to_positions.items():
                print(f"  Símbolo: '{sym}' -> {sorted(pos_set)}")
